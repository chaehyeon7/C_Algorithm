...
int main(void) {
	...
	char* p2;
	char p3[11] = "mirim";		// 최대 '\0'포함하여 11자, 한글은 퇴대 5자까지 가능
	...
	printf("\n[p1이 간접소유한 문자열을 artificail로 변경함]\n");
	printf("\np1이 간접소유한 문자열: %s\n", p1);
	...
	// p3 = "mirimBravo": 불가함. 배열요소는 반복구문 없이 동시 사용은 불가함
	...
	// gets_s()함수는 공백 포함하여 입력받은. 길이 이상 입력하면 입력 취소됨
	gets_s(str[0], 11);
	...
	// 9char 이하이면 '엔터' eh dlqfurqkedma. 11char 이상 입력하여도 10char까지 잘라 입력받음
